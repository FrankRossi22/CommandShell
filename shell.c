...
int main(void)
{
	...
	pid_t pid;
	pid = fork();
	if(pid < 0) { /* Error Occured Creating Fork */
	  fprintf(stderr, "Fork Failed");
	  exit(-1);
	}
	else if(pid == 0) { /* Child Process Calls Command input by User*/
	  execvp(iBuffer, args);
	} else { /* Parent Process Waits if no '&' in User Command, Continues Otherwise*/
	  if(bgrnd == 0) {
	    int opts = 0;
	    waitpid(pid, (int *)NULL, opts);
	  } else {
	    continue;
	  }
	}
